...
const _log = Utils.initLog('Device');
...
export default class Device extends EventEmitter {
    ...
    hasDebugLink: boolean = false;

    firmwareRelease: DeviceTyped.FirmwareRelease | undefined;
    features: DeviceTyped.Features | undefined;
    featuresNeedsReload: boolean = false;

    deferredActions: { [key: string]: CommonTypes.Deferred<void> } = {};
    runPromise: CommonTypes.Deferred<void> | undefined;

    loaded: boolean = false;
    inconsistent: boolean = false;
    firstRunPromise: CommonTypes.Deferred<boolean>;
    ...
    cachedPassphrase: Array<string | undefined> = [];
    ...
    temporaryState: string | undefined;
    ...
    constructor(transport: Transport, descriptor: DeviceDescriptor) {
        ...
        this.hasDebugLink = descriptor.debug === true;

        // this will be released after first run
        this.firstRunPromise = Utils.createDeferred();
    }
    ...
    async acquire(): Promise<void> {
        ...
        this.deferredActions[ DEVICE.ACQUIRE ] = Utils.createDeferred();
        this.deferredActions[ DEVICE.ACQUIRED ] = Utils.createDeferred();
        try {
            ...
            this.deferredActions[ DEVICE.RELEASE ] = Utils.createDeferred();
        } catch (error) {
            this.deferredActions[ DEVICE.ACQUIRED ].resolve();
            delete this.deferredActions[ DEVICE.ACQUIRED ];
            if (this.runPromise) {
    ...
    async run(
        ...
        this.runPromise = Utils.createDeferred(this._runInner.bind(this, fn, options));
    ...
    setInstance(instance: number): void {
        if (this.instance !== instance) {
            ...
            if (this.keepSession) {
                this.activitySessionID = undefined;
                this.keepSession = false;
            }
    ...
    setExpectedState(state?: string): void {
        if (!state) {
            this.setState(undefined); // T2 reset state
            this.setPassphrase(undefined); // T1 reset password
        }
    ...
    clearPassphrase(): void {
        this.cachedPassphrase[ this.instance ] = undefined;
        this.keepSession = false;
    }

    async initialize(useEmptyPassphrase: boolean): Promise<void> {
        if (!this.commands) throw new Error('commands not found');
        const { message }: { message: DeviceTyped.Features } = await this.commands.initialize(useEmptyPassphrase);
    ...
    async getFeatures(): Promise<void> {
        if (!this.commands) throw new Error('commands not found');
        const { message }: { message: DeviceTyped.Features } = await this.commands.typedCall('GetFeatures', 'Features', {});
        this.features = message;
    }
    ...
    isBootloader() {
        return this.features && this.features.bootloader_mode && (typeof this.features.firmware_present === 'boolean' && this.features.firmware_present);
    }
    ...
    hasUnexpectedMode(allow: Array<string>): typeof UI.BOOTLOADER | typeof UI.INITIALIZE | typeof UI.SEEDLESS | null {
        if (this.features) {
    ...
    validateExpectedState(state: string): boolean {
        if (!this.isT1()) {
            const currentState: string | null = this.getExpectedState() || this.getState();
            if (!currentState) {
                this.setState(state);
                return true;
    ...
    getMode() {
        if (!this.features) return 'unknown';
    ...
    toMessageObject(): DeviceTyped.TrezorDevice {
        if (this.originalDescriptor.path === DEVICE.UNREADABLE) {
            ...
            const label = !this.features || this.features.label === '' || this.features.label === null ? defaultLabel : this.features.label;
            return {
                type: 'acquired',
                path: this.originalDescriptor.path,
                label: label,
                ...
                mode: this.getMode(),
                firmware: checkFirmware(this.features),
