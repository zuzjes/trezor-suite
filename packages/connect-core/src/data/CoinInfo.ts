...
import { Coins } from '@trezor/connect';

const bitcoinNetworks: Array<Coins.Bitcoin> = [];
const ethereumNetworks: Array<Coins.Ethereum> = [];
const miscNetworks: Array<Coins.Misc> = [];

export const cloneCoinInfo = <T>(info: T): T => {
    return JSON.parse(JSON.stringify(info));
};

export const getBitcoinNetwork = (pathOrName: Array<number> | string) => {
    const networks: Array<Coins.Bitcoin> = cloneCoinInfo(bitcoinNetworks);
...
export const getEthereumNetwork = (pathOrName: Array<number> | string) => {
    const networks: Array<Coins.Ethereum> = cloneCoinInfo(ethereumNetworks);
    if (typeof pathOrName === 'string') {
        const name: string = pathOrName.toLowerCase();
...
export const getMiscNetwork = (pathOrName: Array<number> | string) => {
    const networks: Array<Coins.Misc> = cloneCoinInfo(miscNetworks);
    if (typeof pathOrName === 'string') {
        const name: string = pathOrName.toLowerCase();
        return networks.find(n => n.name.toLowerCase() === name || n.shortcut.toLowerCase() === name);
    } else {
...
export const getSegwitNetwork = (coin: Coins.Bitcoin): Coins.Bitcoin['network'] | null => {
...
export const getBech32Network = (coin: Coins.Bitcoin): Coins.Bitcoin['network'] | null => {
...
export const fixCoinInfoNetwork = (ci: Coins.Bitcoin, path: Array<number>) => {
...
const detectBtcVersion = (data: any): string => {
...
export const getCoinInfoByHash = (hash: string, networkInfo: any): Coins.Bitcoin => {
    const networks: Array<Coins.Bitcoin> = cloneCoinInfo(bitcoinNetworks);
    ...
    if (result.isBitcoin) {
        ...
        let fork: Coins.Bitcoin | undefined;
        if (btcVersion === 'bch') {
...
export const getCoinInfo = (currency: string) => {
    let coinInfo: Coins.Info | undefined = getBitcoinNetwork(currency);
    if (!coinInfo) {
...
const parseBitcoinNetworksJson = (json: JSON) => {
    ...
    Object.keys(coinsObject).forEach(key => {
        ...
        const network: Coins.Bitcoin['network'] = {
