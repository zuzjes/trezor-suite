...
const PATH_NOT_VALID = ERRORS.invalidParameter('Not a valid path.');
const PATH_NEGATIVE_VALUES = ERRORS.invalidParameter('Path cannot contain negative values.');
...
export const getScriptType = (path?: Array<number>): Protobuf.InputScriptType => {
...
export const getOutputScriptType = (path?: Array<number>): Protobuf.OutputScriptType => {
...
export const getIndexFromPath = (path: Array<number>): number => {
    if (path.length < 3) {
        throw ERRORS.invalidParameter(`getIndexFromPath: invalid path length ${ path.toString() }`);
    }
    return fromHardened(path[2]);
};

export const getAccountLabel = (path: Array<number>, coinInfo: Coins.Bitcoin): string => {
...
export const getPublicKeyLabel = (path: Array<number>, coinInfo?: Coins.Bitcoin): string => {
...
export const getLabel = (label: string, coinInfo?: Coins.Bitcoin): string => {
    if (coinInfo) {
